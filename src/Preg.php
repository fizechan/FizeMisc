...
class Preg
{
     ...
     * 返回匹配模式的数组条目
     *
     * 参数 `$flags` ：如果设置为PREG_GREP_INVERT, 这个函数返回输入数组中与 给定模式pattern不匹配的元素组成的数组.
     ...
     * 使用回调执行正则表达式搜索和替换
     *
     * 参数 `$limit` ：每个主题字符串中每个模式的最大可能替换。默认为-1(没有限制)。
     ...
     *
     * 参数 `$limit` ：对于每个模式用于每个 subject 字符串的最大可替换次数。 默认是-1（无限制）。
